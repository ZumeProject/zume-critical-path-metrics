...
//        }
            ...
            switch ( $administrative_level ) {
                ...
                case 'world':
                    $results = $wpdb->get_results( "
                    SELECT 1 as admin0_grid_id, NULL as admin1_grid_id, NULL as admin2_grid_id, NULL as admin3_grid_id, count('World') as count
                    FROM (
                             SELECT 'World'
                             FROM $wpdb->postmeta as pm
                                      JOIN $wpdb->posts as p ON p.ID=pm.post_id AND p.post_type = 'groups'
                ...
                default:
                    $results = $wpdb->get_results( "
                        SELECT lg.admin0_grid_id, lg.admin1_grid_id, lg.admin2_grid_id, lg.admin3_grid_id, count(lgm.grid_id) as count
                        FROM $wpdb->dt_location_grid_meta lgm
                        JOIN $wpdb->dt_location_grid lg ON lgm.grid_id=lg.grid_id
